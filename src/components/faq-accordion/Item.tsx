    ...
    return (
        <div className='w-full py-4 md:py-6'>
            <button className='w-full text-left flex justify-between items-center gap-8 cursor-pointer group' onClick={onToggle} aria-expanded={isOpen} aria-controls={panelId}>
                <h2 className='md:text-lg font-semibold text-fa-purple-950 transition-colors duration-200 ease-in-out group-hover:text-fa-purple-600'>{title}</h2>
                <div className='relative w-[30px] h-[30px] shrink-0'>
                    <MinusCircleIcon size={30} weight='fill' className={`text-fa-purple-950 absolute top-0 left-0 transition-opacity duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'opacity-0'}`} aria-hidden='true' focusable='false' />
                    <PlusCircleIcon size={30} weight='fill' className={`text-fa-purple-600 absolute top-0 left-0 transition-opacity duration-200 ease-in-out ${isOpen ? 'opacity-0' : 'opacity-100'}`} aria-hidden='true' focusable='false' />
                </div>
            </button>
